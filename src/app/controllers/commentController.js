...
exports.default = async function (req, res) {
    try {
        ...
        try {
            ...
            if(req.query.list=="recent"){
                const rows = await commentDao.defaultDao2(findVideosInfoParams);
                return res.json({
                    data:review
                });
            }
           ...
           const {id} = req.params
            ...
            let review = rows[0][0]
            review.total_review = rows[1]
            review.rating = rows[2]
            review.option = rows[3]
            console.log(review)
            return res.json({
                data:review
            });
